...
class MultisafepayCbc extends PaymentModule
{
    ...
    {
        Configuration::deleteByName('MULTISAFEPAY_CBC_MIN_AMOUNT');
        Configuration::deleteByName('MULTISAFEPAY_CBC_MAX_AMOUNT');
    ...
    public function hookPayment()
    {
        ...
        $min_amount = (int) Configuration::get('MULTISAFEPAY_CBC_MIN_AMOUNT');
        $max_amount = (int) Configuration::get('MULTISAFEPAY_CBC_MAX_AMOUNT');
    ...
    public function getContent()
    {
        if (Tools::isSubmit('submit' . $this->name)) {
            $min_amount = Tools::getValue('MULTISAFEPAY_CBC_MIN_AMOUNT');
            $max_amount = Tools::getValue('MULTISAFEPAY_CBC_MAX_AMOUNT');
            Configuration::updateValue('MULTISAFEPAY_CBC_MIN_AMOUNT', $min_amount);
            Configuration::updateValue('MULTISAFEPAY_CBC_MAX_AMOUNT', $max_amount);
        }
        ...
        $min_amount = Configuration::get('MULTISAFEPAY_CBC_MIN_AMOUNT');
        $max_amount = Configuration::get('MULTISAFEPAY_CBC_MAX_AMOUNT');

        $helper->fields_value['MULTISAFEPAY_CBC_MIN_AMOUNT'] = $min_amount;
        $helper->fields_value['MULTISAFEPAY_CBC_MAX_AMOUNT'] = $max_amount;
        ...
        $fields_form[0]['form'] = [
            ...
            'input' => [
                [
                    ...
                    'label' => $this->l('Minimal order amount for CBC'),
                    'name' => 'MULTISAFEPAY_CBC_MIN_AMOUNT',
                    'required' => false,
                ],
                [
                    'type' => 'text',
                    'class' => 'fixed-width-sm',
                    'prefix' => $this->context->currency->sign,
                    'label' => $this->l('Maximum order amount for CBC'),
                    'name' => 'MULTISAFEPAY_CBC_MAX_AMOUNT',
                    'required' => false,
                ],
            ],
            'submit' => [
